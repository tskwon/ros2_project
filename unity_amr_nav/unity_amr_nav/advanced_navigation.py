...
class AdvancedNavigation(Node):
    ...
    def setup_obstacles(self):
        ...
        self.get_logger().info(f'장애물 설정 완료: x=[{min_x:.2f}, {max_x:.2f}], y=[{min_y:.2f}, {max_y:.2f}]')
    ...
    def plan_path_astar(self, start, goal):
        ...
        if not (0 <= start_grid[0] < self.map_width and 0 <= start_grid[1] < self.map_height):
            self.get_logger().error(f"시작 그리드 ({start_grid})가 맵 범위를 벗어납니다.")
            return None
        if not (0 <= goal_grid[0] < self.map_width and 0 <= goal_grid[1] < self.map_height):
            self.get_logger().error(f"목표 그리드 ({goal_grid})가 맵 범위를 벗어납니다.")
            return None
        
        if self.occupancy_grid[start_grid[1]][start_grid[0]] == 1:
            self.get_logger().error(f"시작점 ({start_grid})이 장애물입니다.")
            return None
        if self.occupancy_grid[goal_grid[1]][goal_grid[0]] == 1:
            self.get_logger().error(f"목표점 ({goal_grid})이 장애물입니다.")
            return None

        self.get_logger().info(f"A* 시작 그리드: {start_grid}, 목표 그리드: {goal_grid}")
        ...
        self.get_logger().warn("A* 알고리즘이 경로를 찾지 못했습니다.")
        return None
    ...
    def publish_path_to_unity(self, path):
        ...
        self.get_logger().info(f'Unity로 경로 전송: {len(path)}개 포인트')
    ...
    def odom_callback(self, msg):
        ...
        self.get_logger().info(
            f'Odom Update: X={self.current_x:.2f}, Y={self.current_y:.2f}, '
            f'Yaw={math.degrees(self.current_yaw):.2f}°, 선속도={self.current_linear_velocity:.2f} m/s'
        )
    ...
    def navigation_loop(self):
        ...
        dynamic_lookahead = max(0.5, min(dynamic_lookahead, 3.0))
    ...
    def get_current_linear_velocity(self):
        return self.current_linear_velocity if self.current_linear_velocity is not None else 0.5
