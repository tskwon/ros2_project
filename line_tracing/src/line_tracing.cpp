...
public:
    LineTracingNode() : Node("line_tracing_node") {
        ...
        stop_signal_pub_ = create_publisher<std_msgs::msg::Bool>("/stop_signal", qos);
        ...
        stop_sub_ = create_subscription<std_msgs::msg::Bool>(
            "/stop_signal", qos, std::bind(&LineTracingNode::stop_callback, this, std::placeholders::_1));
...
private:
    void stop_motors() {
        ...
        std_msgs::msg::Bool stop_signal_msg;
        stop_signal_msg.data = true;
        stop_signal_pub_->publish(stop_signal_msg);

        is_running_ = false;
        current_speed_ = 0.0;
        RCLCPP_INFO(get_logger(), "Motors stopped, stop_signal set to true");
    }
    ...
    void sensor_callback(const std_msgs::msg::Int32MultiArray::SharedPtr msg) {
        ...
        if (active_sensors == 5 && prev_active_sensors_ < 5 && !edge_detect_ && is_running_) {
            edge_detect_ = true;
            stop_motors();
            RCLCPP_WARN(get_logger(), "Rising edge detected → Stopped");
        ...
        if (!is_running_) {
            current_speed_ = 0.0;
        } else if (current_speed_ < base_speed_) {
    ...
    void stop_callback(const std_msgs::msg::Bool::SharedPtr msg) {
        ...
        if (is_running_) {
            edge_detect_ = false;
            RCLCPP_INFO(get_logger(), "Stop signal received → Waiting for rising edge");
        } else {
            RCLCPP_INFO(get_logger(), "Start signal received → Moving");
        }
    }
